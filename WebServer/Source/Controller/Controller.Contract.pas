...
type
    ...
    public
        ...
        [MVCPath('/summary/($ANationalCode)')]
        [MVCHTTPMethods([httpPOST])]
        [MVCProduces(TMVCMediaType.APPLICATION_JSON)]
        [MVCConsumes(TMVCMediaType.APPLICATION_JSON)]
        procedure CreateContractSummary(const ANationalCode: string; const [MVCInject] AContractService: IContractService);
...
//________________________________________________________________________________________
Procedure TContractController.CreateContractSummary(Const ANationalCode: String;
  Const AContractService: IContractService);
Var
    LCreated: TContract;
    LMetaJSON: TJSONObject;
    LStatusCode: Integer;
Begin
    ...
    Try
        Try
            If (ANationalCode.Trim.IsEmpty) then
            Begin
                LStatusCode := HTTP_STATUS.BadRequest;
                raise Exception.Create('کد ملی ورودی نامعتبر است');
            End;

            LCreated := nil;
            try
              // فراخوانی سرویس برای ایجاد مشتری و قرارداد
              LCreated := AContractService.CreateSummery(StrToInt64(ANationalCode));
            Except
            ...
            Try
                ...
                LMetaJSON.AddPair('description', 'قرارداد و مشتری با موفقیت ایجاد شد.');
