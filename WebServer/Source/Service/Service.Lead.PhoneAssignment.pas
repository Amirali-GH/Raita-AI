...
Begin
    ...
    If (LStatusID = 7) then
    Begin
        LRQL := Format('and(or(eq(NationalID,null),eq(NationalID,"")),%s)', [LRQL]);
    End;
...
Begin
    ...
    LRQL := IfThen(LRQL.IsEmpty, EmptyStr, LRQL+';') +
      Format('sort(+Phone);limit(%d,%d)', [LFirstRec, PAGE_SIZE]);

    ATotalSize := TMVCActiveRecord.Count<TPhoneAssignment>(LRQL);
    RESULT := TMVCActiveRecord.SelectRQL<TPhoneAssignment>(LRQL, PAGE_SIZE);
End;
//________________________________________________________________________________________
