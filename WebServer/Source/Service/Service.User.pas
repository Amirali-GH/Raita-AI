...
Begin
    ...
    BEGIN
        If (LHasActive) then
        Begin
            LRQL := Format('and(%s, or(contains(LastName,"%s"), contains(Phone, "%s")))',
              [LRQL, AContext, AContext]);
        End
        Else
        Begin
            LRQL := Format('or(contains(LastName,"%s"), contains(Phone, "%s"))',
              [AContext, AContext]);
        End;

        LFilter := LFilter + ' AND (LastName LIKE ? OR Phone LIKE ?)';
        LParams := LParams + ['%' + AContext + '%', '%' + AContext + '%'];
    END;
